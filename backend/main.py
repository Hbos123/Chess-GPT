...
async def get_positions_by_tag_transition(
    ...
    try:
        ...
        # Backwards compatibility: if legacy rows have NULL error_side, pull those too to fill the page.
...
class BackfillPositionsFromPgnRequest(BaseModel):
    user_id: str
    max_games: int = 200
...
@app.post("/admin/backfill-positions-from-pgn")
async def admin_backfill_positions_from_pgn(request: BackfillPositionsFromPgnRequest):
    """
    Backfill the `positions` table for a user by reconstructing fen_before from PGN,
    using stored ply_records (often compact) for category/cp_loss/tags.

    This is primarily needed for older games whose compact game_review dropped fen_before / engine fields,
    resulting in 0 drill positions available.
    """
    if not supabase_client:
        raise HTTPException(status_code=503, detail="Supabase client not initialized")
    if not engine:
    ...
    uid = request.user_id
    max_games = int(request.max_games or 0)
    max_positions_per_game = int(request.max_positions_per_game or 0)
    min_cp_loss = float(request.min_cp_loss or 0)
    verify_depth = int(request.verify_depth or 12)
    ...
    try:
        ...
        games = await asyncio.to_thread(fetch_games)
        if not games:
            return {"status": "ok", "user_id": uid, "games": 0, "positions_saved": 0, "note": "No analyzed games found"}

        saved_total = 0
        games_processed = 0
        games_with_positions = 0

        async def best_move_for_fen(fen: str) -> tuple[str, str, float]:
            board = chess.Board(fen)
            info = await engine.analyse(board, chess.engine.Limit(depth=verify_depth), multipv=1)
            pv = info["pv"]
            move = pv[0]
        ...
        def infer_category(rec: Dict) -> str:
            ...
            if cat in ("blunder", "mistake", "inaccuracy"):
                return cat
            # Frontend may omit category; infer from analyse.tags or boolean fields.
        ...
        for g in games:
            ...
            for idx, rec in enumerate(ply_records):
                if not isinstance(rec, dict):
                    continue
                ply = rec.get("ply")
                try:
                    ply_i = int(ply) if ply is not None else (idx + 1)
                except Exception:
                    ply_i = idx + 1
                ...
                move_san = rec.get("san") or san_moves[ply_i - 1]
                ...
                tags_start = extract_names(raw_before.get("tags")) if raw_before else []
                tags_after_played = extract_names(raw_after.get("tags")) if raw_after else extract_names(analyse.get("tags"))
                ...
                try:
                    best_san, best_uci, eval_cp = await best_move_for_fen(fen_before)
                except Exception:
                    continue
            ...
            saved_count = await asyncio.to_thread(
                supabase_client.batch_upsert_positions, uid, positions_to_save, game_id
            )
        ...
        import traceback
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail=f"Failed to backfill positions: {str(e)}")
