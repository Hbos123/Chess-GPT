...
Write your response now based on the game data provided."""
    ...
    async def _generate_table(self, args: Dict, context: Dict = None) -> Dict:
        """
        Generate comparison/analysis tables from game data.
        """
        table_type = args.get("table_type", "opening_comparison")
        games_data = args.get("games", [])  # Array of games from frontend or reviews
        filters = args.get("filters", {})
        
        if not games_data:
            return {"error": "No games data provided"}
        
        if table_type == "opening_comparison":
            # Group by opening, calculate stats
            opening_stats = {}
            for game in games_data:
                ...
                key = f"{opening}_{eco}" if eco else opening
                ...
                if accuracy is not None:
                    opening_stats[key]["accuracy"].append(float(accuracy))
            
            # Build table rows
            rows = []
            for key, stats in opening_stats.items():
                avg_acc = sum(stats["accuracy"]) / len(stats["accuracy"]) if stats["accuracy"] else None
                total_decisive = stats["wins"] + stats["losses"]
                win_rate = (stats["wins"] / total_decisive * 100) if total_decisive > 0 else 0
                
                rows.append({
                    "opening": f"{stats['opening']} ({stats['eco']})" if stats['eco'] else stats['opening'],
                    "games": stats["games"],
                    "wins": stats["wins"],
                    "losses": stats["losses"],
                    "draws": stats["draws"],
                    "win_rate": f"{win_rate:.1f}%",
                    "avg_accuracy": f"{avg_acc:.1f}%" if avg_acc is not None else "N/A"
                })
            ...
            return {
                "success": True,
                "table_type": "opening_comparison",
                "headers": ["Opening", "Games", "Wins", "Losses", "Draws", "Win Rate", "Avg Accuracy"],
                "rows": rows,
                "title": "Opening Performance Comparison"
            }
        
        elif table_type == "time_control_stats":
            ...
            for game in games_data:
                ...
                if accuracy is not None:
                    tc_stats[tc]["accuracy"].append(float(accuracy))
            ...
            return {
                "success": True,
                "table_type": "time_control_stats",
                "headers": ["Time Control", "Games", "Win Rate", "Avg Accuracy"],
                "rows": rows,
                "title": "Performance by Time Control"
            }
        
        elif table_type == "color_comparison":
            # Compare White vs Black performance
            white_stats = {"games": 0, "wins": 0, "losses": 0, "draws": 0, "accuracy": []}
            black_stats = {"games": 0, "wins": 0, "losses": 0, "draws": 0, "accuracy": []}
            ...
            for game in games_data:
                ...
                stats = white_stats if color == "white" else black_stats if color == "black" else None
                if not stats:
                    continue
            ...
            return {
                "success": True,
                "table_type": "color_comparison",
                "headers": ["Color", "Games", "Wins", "Losses", "Draws", "Win Rate", "Avg Accuracy"],
                "rows": rows,
                "title": "White vs Black Performance"
            }
